...
export const BookEditor: React.FC<{
  ...
  const generateAndSaveChapterSummary = async (
    ...
    try {
      ...
      setSummaryErrors(prev => [...prev, `Failed to generate summary: ${error?.message || 'Unknown error'}`]);
    }
  };
  ...
  const generateNewChapter = async () => {
    ...
    try {
      ...
      try {
        ...
        setSummaryErrors(prev => [...prev, `Failed to generate summary for chapter ${idx}: ${summaryError?.message || 'Unknown error'}`]);
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-blue-50 to-indigo-100">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {/* summary errors banner */}
        {summaryErrors.length > 0 && (
          <div className="bg-amber-50 border border-amber-300 text-amber-800 px-4 py-3 rounded-lg mb-4">
            <div className="flex items-start gap-2">
              <svg className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
              </svg>
              <div>
                <div className="font-medium">Chapter Summary Warnings</div>
                <div className="text-sm mt-1">
                  Some chapter summaries couldn't be generated:
                </div>
