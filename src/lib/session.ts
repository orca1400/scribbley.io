...
function safeUuid(): string {
  try {
    ...
    // Use getRandomValues as a fallback if randomUUID is not available
    if (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function') {
      // RFC4122 version 4 UUID, using getRandomValues
  ...
  // If neither crypto.randomUUID nor crypto.getRandomValues is available, throw
  throw new Error("No secure random API available for UUID generation.");
}
